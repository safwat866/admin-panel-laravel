...
<div class="content-body">
    <div class="table_buttons mb-1">
        <div class="flex items-center">
            ...
            <form action="{{ route("roles.bulkDelete") }}" method="post" id="bulk_delete">
                @csrf
    ...
    <div class="table_container">
        <table class="bordered-table">
            ...
            <tbody>
                ...
                <tr>
                    <th><input type="checkbox" value="{{ $role->id }}" class="product_checkbox"></th>
                    <th>{{ $role->id }}</th>
                    <th>{{ $role->name }}</th>
                    <th class="product-action">
                        <a class="btn rounded-pill btn-primary" href="{{ route('roles.edit', $role->id) }}">
                            <i class="fa fa-edit"></i>
                        </a>
                        @if ($role->name != 'Admin')
                        <a class="delete-row btn rounded-pill btn-danger mx-2 text-white" onclick="event.preventDefault();document.getElementById('destroy-form').submit()"><i class="fa fa-trash"></i></a>
                        <form action="{{ route('roles.destroy', $role->id) }}" id="destroy-form" method="POST">
                            @csrf
                            @method("DELETE")
                        </form>
                        @endif
    ...
    <script>
        ...
        $("#bulk_delete").on("submit", (e) => {
            ...
            productsId.forEach((value, index) => {
                let inputs = `<input type="hidden" name="roles[]" value="${value}"/>`;
